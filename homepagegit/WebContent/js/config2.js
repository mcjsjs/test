...
jQuery.fn.h5u_xcss = function(k, v) {
	return $(this).css('-webkit-' + k, v).css('-moz-' + k, v).css('-o-' + k, v)
			.css('-ms-' + k, v).css(k, v);
};
...
var h5u_parallelism = (function($) {
	var _ = {
		...
		settings : {
			...
			popupOverlayOpacity : 0.75, // Popup overlay opacit
			// Mobile only
		...
		initDesktop : function() {
			...
			var windowHeight = _.objects.window.height() - _.settings.marginTop
					- _.settings.marginBottom, windowWidth = _.objects.window
					.width(), itemHeight = _.settings.itemHeight, itemCount = _.objects.items.length, itemsWidth = 0, rows = 0, rowWidth, SZIntervalId;
			...
			_.objects.window._parallelism_update = function() {
				...
				rows = Math.min(Math.max(
						Math.floor(windowHeight / itemHeight) - 1, 1),
						_.settings.maxRows);

				// Reduce row count if we have more than we need
				while (rows > _.settings.minRows
						&& (itemsWidth / rows) < windowWidth)
					rows--;
				...
				// Resize reel
				_.objects.reel.css('height',
						(itemHeight * rows) + (_.settings.padding * 2)).css(
						'width', rowWidth);

				// Reposition main (if applicable)
				_.objects.main.css('height', (itemHeight * rows)
						+ (_.settings.padding * 2));

				if (_.settings.centerVertically)
					_.objects.main.css('top', '50%').css(
							'margin-top',
							(-1 * (_.objects.main.outerHeight() / 2))
									+ _.settings.marginTop
									- _.settings.marginBottom);

				// Resize/reposition SZs
				window.setTimeout(function() {
					$SZ.css('height', _.objects.main.outerHeight()).css('top',
							_.objects.main.offset().top);
			...
			_.objects.window
					.resize(function() {
						...
						windowHeight = _.objects.window.height()
								- _.settings.marginTop
								- _.settings.marginBottom;

						// Row count changed? Re-update
						if (Math.max(Math.floor(windowHeight / itemHeight) - 1,
								1) != rows)
			...
			// Reel
			_.objects.reel.css('overflow-y', 'hidden').css('margin', '0 auto')
					.css(
							'border',
							'solid ' + _.settings.padding + 'px '
									+ _.settings.paddingColor).css(
							'box-shadow',
							'inset 0px 0px 0px ' + _.settings.padding + 'px '
									+ _.settings.paddingColor);
			...
			if (_.IEVersion < 10) {
				...
				window.setTimeout(function() {
					_.objects.reel.h5u_xcss(
							'transition',
							'opacity ' + (_.settings.introSpeed / 1000.00)
									+ 's ease-in-out').css('opacity', 1);
				}, _.settings.introDelay);
			}
			...
			_.objects.items.css(
					'box-shadow',
					'0px 0px 0px ' + _.settings.padding + 'px '
							+ _.settings.paddingColor).css(
					'border',
					'solid ' + _.settings.padding + 'px '
							+ _.settings.paddingColor);
			...
			_.objects.items.each(function(i) {
				...
				// Item
				$item.css('position', 'relative').css('width', w).css('height',
						h);
				...
				if ($img.length > 0) {
					...
					// img
					$img.css('position', 'absolute').css('width', '100%').css(
							'height', 'auto').css('min-height', '100%').css(
							'top', 0).css('left', 0)
					...
					$itemInner.css('position', 'relative').css('display',
							'block').css('-webkit-backface-visibility',
					...
					if (_.IEVersion < 9)
						...
						$img.load(function() {
							...
							}, _.settings.thumbDelay
									+ Math.floor(Math.random()
											* _.settings.thumbDelaySpread));
						});
					} else {
						$itemInner.css('opacity', 0);

						$img.load(function() {
							$itemInner.h5u_xcss('transition', 'opacity '
									+ (_.settings.thumbSpeed / 1000.00)
									+ 's ease-in-out');
			...
			// Main
			if (_.isTouch)
				_.objects.main.css('overflow-x', 'auto').css('overflow-y',
						'hidden').h5u_xcss('overflow-scrolling', 'touch');
			else
			...
			if (_.IEVersion < 9)
				...
				var scrollHandler = function(e) {
					var delta = (e.detail ? e.detail * -10 : e.wheelDelta)
							* _.settings.scrollFactor;
					_.objects.main.scrollLeft(_.objects.main.scrollLeft()
							- delta);
					$SZ._parallelism_update();
					e.preventDefault();
					e.stopPropagation();
			...
			if (!_.isTouch && _.settings.useScrollZones) {
				_.objects.body
						.append('<div class="SZRight" style="right: 0;" />');
				_.objects.body
						.append('<div class="SZLeft" style="left: 0;" />');

				$SZLeft = _.objects.body.children('.SZLeft');
				$SZRight = _.objects.body.children('.SZRight');
				$SZ = $SZLeft.add($SZRight);

				$SZ.css('position', 'fixed').css('width',
						_.settings.scrollZoneWidth).css('height', 100).css(
						'z-index', 100)
				...
				$SZ._parallelism_update = function() {
					...
					if (_.objects.main.scrollLeft() + $(window).width() >= _.objects.reel
							.outerWidth())
						$SZRight.hide();
					else
						$SZRight.show();
								...
								function(e) {
									SZIntervalId = window
											.setInterval(
													function() {
														_.objects.main
																.scrollLeft(_.objects.main
																		.scrollLeft()
																		+ (_.settings.scrollZoneAmount * _.settings.scrollFactor));
														$SZ
																._parallelism_update();
													},
				...
				_.objects.window
						.keydown(function(e) {
							...
							case 39:
								window.clearInterval(SZIntervalId);
								_.objects.main
										.scrollLeft(_.objects.main.scrollLeft()
												+ (_.settings.scrollKeyAmount * _.settings.scrollFactor));
								$SZ._parallelism_update();
								return false;
		...
		initMobile : function() {
			...
			if (_.settings.autoStyleMobile) {
				...
				_.objects.reel.css(
						'border',
						'solid ' + Math.ceil(_.settings.padding / 2) + 'px '
								+ _.settings.paddingColor).css('border-bottom',
						0);

				// Items
				_.objects.items.css('border-bottom', 'solid '
						+ Math.ceil(_.settings.padding / 2) + 'px '
						+ _.settings.paddingColor);

				_.objects.items.filter('.thumb').find('img').css('width',
						'100%').css('height', 'auto');
			}
		...
		init : function(userSettings) {
			...
			// Get objects
			_.objects.window = $(window), _.objects.wrapper = $('#wrapper'),
					_.objects.body = $('body'), _.objects.main = $('#main'),
					_.objects.reel = $('#reel'),
